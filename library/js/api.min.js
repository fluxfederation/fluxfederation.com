...
$(document).ready(function(){$(".events-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addEventsToPage(a)):"",$(".blog-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addBlogsToPage(a)):"",$(".blog-post-page").length?function(){const a=()=>new URL(document.location).searchParams.get("id");return console.log("test"),a()?new Promise((b,c)=>{$.ajax({url:`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/post/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void(window.location.href="../")}().then(a=>addBlogToPage(a)):"",addEventsToPage=a=>{html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</article>`,$(".events-page").append(html)},addBlogToPage=a=>{console.log(a),$(".js-blog-title").text(a.post_title)},addBlogsToPage=a=>{html=`<article class="wrap cf">`,n=0,$.each(a.reverse(),(a,b)=>{n++,1==n?html+=`<div class="space-top-double" style="display: inline-block; position: relative;">`:"",html+=blogItem(b),3==n?html+=`</div>`:"",3==n?n=0:""}),html+=`</article>`,$(".blog-page").append(html)},eventItem=a=>(console.log(a),html=`<div class="m-all t-1of4">
				<a style="text-decoration:none;" href="event/?id=${a.ID}">
					<img src="${a.image_url}" style="padding: 30px;object-fit: contain; height: 200px;">
					<h4>${a.title}</h4>
				</a>
				<div class="cf" style="margin-top: 20px; bottom: 0;">
					<strong><p style="float: left; margin: 0;">${a.dates}</p></strong>
					<br>
					<p style="opacity: 0.7; float: left; margin: 0;">${a.blurb}</p>
				</div>
			</div>`,html),blogItem=a=>(console.log(a),date_written=a.date_written.split("/"),date=new Date(parseInt(date_written[0]),parseInt(date_written[1]),parseInt(date_written[2])),day=date.getDate(),month=date.toLocaleString("default",{month:"short"}),html=`<div class="m-all t-4of12">
				<a style="text-decoration:none;" href="post?id=${a.ID}">
					<img src="${a.banner_image}" style="object-fit: cover; height: 190px; background-color: rgba(0,0,0,0.05);">
					<h4>${a.post_title}</h4>
				</a>
				<div class="cf" style="margin-top: 20px; bottom: 0;">
					<image style="float: left; height: 44px; width: 44px; object-fit: cover; border-radius: 50%;" src="${a.author_image}"">
					<strong><p style="float: left; margin: 0 10px;">${a.author_name}</p></strong>
					<br>
					<p style="opacity: 0.7; float: left; margin: 0 10px;">${month} ${day} | ${a.read_time} min read</p>
				</div>
			</div>`,html)});
