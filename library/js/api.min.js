...
$(document).ready(function(){$(".case-studies-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://cms.fluxfederation.com/wp-json/fluxapi/v1/case-studies",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addCaseStudyToPage(a)):""});
...
$(document).ready(function(){const a=this;console.log("test"),$(".events-page").length&&function(){return console.log("test"),new Promise((a,b)=>{$.ajax({url:"https://cms.fluxfederation.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>{// events.length = 0
a.length?addEventsToPage(a):$(".no-events-banner").fadeIn()}),$(".blog-page").length&&function(a="init"){return url=`https://cms.fluxfederation.com/wp-json/fluxapi/v1/posts`,new Promise((a,b)=>{$.ajax({url:url,crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(b=>{a.blogs=b,$(".skeleton-new-items").remove(),addBlogsToPage()}),$(".single-blog-page").length?function(){const a=()=>new URL(document.location).searchParams.get("name");return a()?new Promise((b,c)=>{$.ajax({url:`https://cms.fluxfederation.com/wp-json/fluxapi/v1/post/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void(window.location.href="../")}().then(a=>addBlogToPage(a)):"",$(".social-share-container li").click(a=>{switch(current_url=window.location.href,encoded_url=encodeURIComponent(current_url),test_encoded_url=encodeURIComponent("https://fluxfederation.com/"),encoded_title=encodeURIComponent(document.title),facebook_url=`https://www.facebook.com/sharer/sharer.php?u=${encoded_url}&t=${encoded_title}`,twitter_url=`http://twitter.com/share?text=${document.title}&url=${current_url}`,linkedin_url=`https://www.linkedin.com/shareArticle?mini=true&url=${test_encoded_url}&title=${encoded_title}`,a.currentTarget.id){case"facebook":window.open(facebook_url,"pop","width=600, height=400, scrollbars=no");break;case"twitter":window.open(twitter_url,"pop","width=600, height=400, scrollbars=no");break;case"linkedin":window.open(linkedin_url,"pop","width=600, height=400, scrollbars=no");break;case"mail":$(`<a href="mailto:?Subject=${encoded_title}&Body=${encoded_url}"></a>`)[0].click();}}),$(document).on("click",".show-more-blogs .flux-button",()=>addBlogsToPage()),addEventsToPage=a=>{console.log(a),html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</article>`,$(".events-page").append(html)},seperateDateString=a=>(a=a.split("/"),console.log(a),a=new Date(parseInt(a[2]),parseInt(a[1])-1,parseInt(a[0])),day=a.getDate(),month=a.toLocaleString("default",{month:"short"}),{month:month,day:day}),returnToBlogLink=()=>`<a href="../" class="back-to-blog-link m-all t-5of6"><p>< Back to Blog</p></a>`,addBlogToPage=a=>{""==a.post_content?window.location.href="../":"",$(".blog-title").text(a.post_title),$(blogPostMeta(a)).insertAfter(".blog-title"),$(".banner-image").attr("src",a.banner_image),$(".banner-image-caption").text(a.banner_image_caption),$(".blog-content article").html(a.post_content),$(".fb-share-button").data("href",window.location.href),$.each(a.tags,(a,b)=>{$(".tags-container").append(blogTag(b.name))}),$.each(a.other_posts,(a,b)=>{$(".other-posts").append(otherBlogPreview(b))}),document.title=`${a.post_title} - Flux Federation`},blogTag=a=>`<div class="tag">${a}</div>`,addBlogsToPage=()=>{console.log(a.blogs),$(".show-more").remove(),html=``,n=0;for(var c=0;6>c;c++)a.blogs.length&&(n++,blog=a.blogs[0],1==n?html+=`<div class="news-items-row">`:"",html+=indexBlogPreview(blog),3==n||1==a.blogs.length?html+=`</div>`:"",3==n?n=0:"",a.blogs.splice(0,1));a.blogs.length?html+=b():"",$(".blog-section").append(html)};const b=()=>`<div class="show-more show-more-blogs"><div class="flux-button"><a>Show More</a></div></div>`;eventItem=a=>(date=seperateDateString(a.start_date),html=`<div class="m-all t-4of12 news-item">
			<img src="" class="item-image item-image-logo">
			<h4>${a.title}</h4>
			<div class="cf item-info">
				<strong><p>${date.month} ${date.day}</p></strong>
				<strong><p><a target="_blank" href="http://www.google.com/maps/place/${a.location.lat},${a.location.lng}">${a.location.address.split(",")[0]}</a></p></strong>
				<br>
				<p class="preview">${a.description}</p>
			</div>
		</div>`,html),blogPostMeta=a=>(console.log(a),date=seperateDateString(a.date_written),html=`<div class="cf item-info">
			<strong><p class="author">By ${a.author_name}</p></strong>
			<br>
			<p class="item-meta-info">${date.month} ${date.day} | ${a.read_time} min read</p>
		</div>`,html);const c=a=>`<div class="flux-button news-item-button"><a href="/newsroom/blog/post/?name=${a}">Read More</a></div>`;indexBlogPreview=a=>(html=`<div class="m-all t-4of12 news-item">
			...
			${c(a.post_name)}
		</div>`,html),otherBlogPreview=a=>(html=`<div class="news-item">
