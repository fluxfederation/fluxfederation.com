...
$(document).ready(function(){$(".events-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addEventsToPage(a)):"",$(".blog-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addBlogsToPage(a)):"",addEventsToPage=a=>{html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</article>`,$(".events-page").append(html)},addBlogsToPage=a=>{html=`<article class="wrap cf">`,n=0,$.each(a.reverse(),(a,b)=>{n++,1==n?html+=`<div class="space-top-double" style="display: inline-block; position: relative;">`:"",html+=blogItem(b),3==n?html+=`</div>`:"",3==n?n=0:""}),html+=`</article>`,$(".blog-page").append(html)},eventItem=a=>(console.log(a),html=`<div class="m-all t-1of4">
				<a style="text-decoration:none;" href="/event/?id=${a.title.toLowerCase().replace(/[^\w ]+/g,"").replace(/ +/g,"-")}">
				...
				<a style="text-decoration:none; href="/blog/post?id=${a.ID}">
