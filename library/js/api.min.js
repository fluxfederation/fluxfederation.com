...
$(document).ready(function(){function a(a="init"){return url="init"==a?`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts`:`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts/${a}`,new Promise((a,b)=>{$.ajax({url:url,crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}$(".events-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addEventsToPage(a)):"",$(".blog-page").length?a().then(a=>addBlogsToPage(a)):"",$(".single-blog-page").length?function(){const a=()=>new URL(document.location).searchParams.get("id");return console.log("test"),a()?new Promise((b,c)=>{$.ajax({url:`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/post/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void(window.location.href="../")}().then(a=>addBlogToPage(a)):"",$(document).on("click",".show-more-blogs .flux-button",b=>a($(b.target).data("page-n")).then(a=>addBlogsToPage(a))),addEventsToPage=a=>{html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</article>`,$(".events-page").append(html)},blogPostMeta=a=>(date_written=a.date_written.split("/"),date=new Date(parseInt(date_written[0]),parseInt(date_written[1]),parseInt(date_written[2])),day=date.getDate(),month=date.toLocaleString("default",{month:"short"}),html=`<div class="cf item-info">
		...
		</div>`,html),blogItem=a=>(console.log(a),html=`<div class="m-all t-4of12 news-item">
			<a href="post?id=${a.ID}">
