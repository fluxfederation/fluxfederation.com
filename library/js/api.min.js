...
$(document).ready(function(){const a=this;$(".events-page").length&&function(){return new Promise((a,b)=>{$.ajax({url:"https://cms.fluxfederation.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>{a.length?c(a):$(".no-events-banner").fadeIn()}),$(".blog-page").length&&function(a="init"){return url=`https://cms.fluxfederation.com/wp-json/fluxapi/v1/posts`,new Promise((a,b)=>{$.ajax({url:url,crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(b=>{a.blogs=b,$(".skeleton-new-items").remove(),h()}),$(".single-blog-page").length?function(){const a=()=>new URL(document.location).searchParams.get("name");return a()?new Promise((b,c)=>{$.ajax({url:`https://cms.fluxfederation.com/wp-json/fluxapi/v1/post/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void b()}().then(a=>e(a)):"",$(".social-share-container li").click(a=>{switch(current_url=window.location.href,encoded_url=encodeURIComponent(current_url),test_encoded_url=encodeURIComponent("https://fluxfederation.com/"),encoded_title=encodeURIComponent(document.title),facebook_url=`https://www.facebook.com/sharer/sharer.php?u=${encoded_url}&t=${encoded_title}`,twitter_url=`http://twitter.com/share?text=${document.title}&url=${current_url}`,linkedin_url=`https://www.linkedin.com/shareArticle?mini=true&url=${test_encoded_url}&title=${encoded_title}`,a.currentTarget.id){case"facebook":window.open(facebook_url,"pop","width=600, height=400, scrollbars=no");break;case"twitter":window.open(twitter_url,"pop","width=600, height=400, scrollbars=no");break;case"linkedin":window.open(linkedin_url,"pop","width=600, height=400, scrollbars=no");break;case"mail":$(`<a href="mailto:?Subject=${encoded_title}&Body=${encoded_url}"></a>`)[0].click();}}),$(document).on("click",".show-more-blogs .flux-button",()=>h());const b=()=>window.location.href="../",c=a=>{console.log(a),html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=k(b)),html+=`</article>`,$(".events-page").append(html)},d=a=>(a=a.split("/"),a=new Date(parseInt(a[2]),parseInt(a[1])-1,parseInt(a[0])),day=a.getDate(),month=a.toLocaleString("default",{month:"short"}),{month:month,day:day}),e=a=>{""==a.post_content?b():"",$(".blog-title").text(a.post_title),$(m(a)).insertAfter(".blog-title"),$(".banner-image").attr("src",a.banner_image),$(".banner-image-caption").text(a.banner_image_caption),$(".blog-content article").html(a.post_content),$(".fb-share-button").data("href",window.location.href),$.each(a.tags,(a,b)=>$(".tags-container").append(g(b.name))),$.each(a.other_posts,(a,b)=>$(".other-posts").append(q(b))),f(a.post_title)},f=a=>document.title=`${a} - Flux Federation`,g=a=>`<div class="tag">${a}</div>`,h=()=>{console.log(a.blogs),$(".show-more").remove(),html=``,n=0;for(var b=0;6>b;b++)a.blogs.length&&(n++,blog=a.blogs[0],1==n?html+=`<div class="news-items-row">`:"",html+=p(blog),3==n||1==a.blogs.length?html+=`</div>`:"",3==n?n=0:"",a.blogs.splice(0,1));a.blogs.length?html+=j():"",$(".blog-section").append(html)},j=()=>`<div class="show-more show-more-blogs"><div class="flux-button"><a>Show More</a></div></div>`,k=a=>(date=d(a.start_date),html=`<div class="m-all t-4of12 news-item">
			<img src="" class="item-image item-image-logo">
			<h4>${a.title}</h4>
			<div class="cf item-info">
				<strong><p>${date.month} ${date.day}</p></strong>
				<strong><p><a target="_blank" href="${l(a.location.lat,a.location.lng)}">${a.location.address.split(",")[0]}</a></p></strong>
				<br>
				<p class="preview">${a.description}</p>
			</div>
		</div>`,html),l=(a,b)=>`http://www.google.com/maps/place/${a},${b}`,m=a=>(console.log(a),date=d(a.date_written),html=`<div class="cf item-info">
