...
$(document).ready(function(){$(".case-studies-page").length&&function(){return new Promise((a,b)=>{$.ajax({url:"https://cms.fluxfederation.com/wp-json/fluxapi/v1/case-studies",crossDomain:!0,success:b=>a(b),error:a=>b(a)})})}().then(b=>{self.case_studies=b,a(b)}),$(".single-case-study-page").length?function(){const a=()=>new URL(document.location).searchParams.get("name");return a()?new Promise((b,c)=>{$.ajax({url:`https://cms.fluxfederation.com/wp-json/fluxapi/v1/case-study/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void redirectBack()}().then(a=>d(a)):"";const a=(a=self.case_studies,c=$(".case-studies-section"))=>{html=``,n=0;for(var d=0;6>d;d++)a.length&&(n++,item=a[0],1==n?html+=`<div class="news-items-row">`:"",html+=b(item),3==n||1==a.length?html+=`</div>`:"",3==n?n=0:"",a.splice(0,1));c.append(html)},b=a=>(console.log(a),html=`<div class="m-all t-4of12 news-item">
			...
			${c(a.post_name)}
		</div>`,html),c=a=>`<div class="flux-button news-item-button"><a href="/solutions/case-studies/post?name=${a}">Read More</a></div>`,d=a=>{// $('.banner-image').attr('src', case_study.banner_image)
...
""==a.post_content?redirectBack():"",$(".case-study-title").text(a.post_title),console.log(a.post_content),$(".case-study-content article").html(a.post_content),e(a.post_title)},e=a=>document.title=`${a} - Flux Federation`});
