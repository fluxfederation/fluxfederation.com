...
$(document).ready(function(){$(".case-studies-page").length?function(){return console.log("testt"),new Promise((a,b)=>{$.ajax({url:"https://cms.fluxfederation.com/wp-json/fluxapi/v1/case-studies",crossDomain:!0,success:b=>a(b),error:a=>b(a)})})}().then(b=>a(b)):"";const a=()=>{console.log(self.blogs),$(".show-more").remove(),html=``,n=0;for(var a=0;6>a;a++)self.blogs.length&&(n++,blog=self.blogs[0],1==n?html+=`<div class="news-items-row">`:"",html+=indexBlogPreview(blog),3==n||1==self.blogs.length?html+=`</div>`:"",3==n?n=0:"",self.blogs.splice(0,1));self.blogs.length?html+=showMorePostsButton():"",$(".blog-section").append(html)}});
...
$(".blog-section").append(html)},j=a=>(date=e(a.start_date),html=`<div class="m-all t-4of12 news-item">
			...
			${m(a.post_name)}
		</div>`,html),p=a=>(html=`<div class="news-item">
