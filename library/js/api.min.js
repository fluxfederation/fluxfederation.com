...
$(document).ready(function(){$(".events-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addEventsToPage(a)):"",$(".blog-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addBlogsToPage(a)):"",addEventsToPage=a=>{html=`<section class="white"><article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</section>`,$(".events-page").append(html)},addBlogsToPage=a=>{html=`<section class="white"><article class="wrap cf">`,n=0,$.each(a.reverse(),(a,b)=>{n++,1==n?html+=`<div class="space-top-double" style="display: inline-block; position: relative;">`:"",html+=blogItem(b),3==n?html+=`</div>`:"",3==n?n=0:""}),html+=`</section>`,$(".blog-page").append(html)},eventItem=a=>(html=`<div class="table-layout">
				<div class="m-all t-4of12">
					<img src="${a.image_url}" style="object-fit: cover; max-height: 200px;">
				</div>
				...
				<a href="/blog/post?id=${a.ID}">
					<img src="${a.banner_image}" style="object-fit: cover; max-height: 200px;">
					<h4 style="text-decoration:none;">${a.post_title}</h4>
				</a>
