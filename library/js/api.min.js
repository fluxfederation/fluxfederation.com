...
$(document).ready(function(){const a=this;$(".events-page").length?function(){return new Promise((a,b)=>{$.ajax({url:"https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/events",crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(a=>addEventsToPage(a)):"",$(".blog-page").length&&function(a="init"){return url=`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/posts`,new Promise((a,b)=>{$.ajax({url:url,crossDomain:!0,success:function(b){a(b)},error:function(a){b(a)}})})}().then(b=>{a.blogs=b,addBlogsToPage()}),$(".single-blog-page").length?function(){const a=()=>new URL(document.location).searchParams.get("id");return console.log("test"),a()?new Promise((b,c)=>{$.ajax({url:`https://fluxfederation.wpengine.com/wp-json/fluxapi/v1/post/${a()}`,crossDomain:!0,success:function(a){b(a)},error:function(a){c(a)}})}):void(window.location.href="../")}().then(a=>addBlogToPage(a)):"",$(document).on("click",".show-more-blogs .flux-button",()=>addBlogsToPage()),addEventsToPage=a=>{html=`<article class="wrap cf">`,$.each(a.reverse(),(a,b)=>html+=eventItem(b)),html+=`</article>`,$(".events-page").append(html)},blogPostMeta=a=>(date_written=a.date_written.split("/"),date=new Date(parseInt(date_written[0]),parseInt(date_written[1]),parseInt(date_written[2])),day=date.getDate(),month=date.toLocaleString("default",{month:"short"}),html=`<div class="cf item-info">
		...
		</div>`,html),returnToBlogLink=()=>`<a href="../" class="back-to-blog-link m-all t-5of6"><p>< Back to Blog</p></a>`,calcPageNum=()=>0==$(".news-item").length%6?$(".news-item").length?$(".news-item").length/6:1:0,addBlogToPage=a=>{console.log(a),""==a.post_content?window.location.href="../":"",$(".blog-title").text(a.post_title),$(blogPostMeta(a)).insertAfter(".blog-title").addClass("m-all t-5of6").after($(returnToBlogLink())),$(".banner-image").attr("src",a.banner_image),$(".banner-image-caption").text(a.banner_image_caption),$(".blog-content article").html(a.post_content).after(returnToBlogLink()),$(".fb-share-button").data("href",window.location.href),document.title=`${a.post_title} - Flux Federation`},addBlogsToPage=()=>{$(".show-more").remove(),html=``,n=0;for(var b=0;6>b;b++)a.blogs.length&&(n++,console.log(a.blogs),blog=a.blogs[0],1==n?html+=`<div class="news-items-row">`:"",html+=blogItem(blog),3==n||1==a.blogs.length?html+=`</div>`:"",3==n?n=0:"",a.blogs.splice(0,1));a.blogs.length?html+=`<div class="show-more show-more-blogs"><div class="flux-button"><a>Show More</a></div></div>`:"",$(".blog-section").append(html)},eventItem=a=>(console.log(a),html=`<div class="m-all t-4of12 news-item">
